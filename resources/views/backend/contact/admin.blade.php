                    ...
                    <div class="media">
                        <div class="media-body p-15px-l lh-normal">
                            ...
                            <form id="formContact" data-form-output="form-output-global" data-form-type="contact" method="post" action="{{$action}}">
                                ...
                                <input type="hidden" id="to_id" value="@if($role==='admin') 1  @elseif($role==='afa') @if(Auth::user()->hasAfa()){{Auth::user()->afa_id}} @else{{App\Models\User::where('name',Request::get('afa'))->first()->id}}@endif @else {{ Auth::user()->apl_id }} @endif">
                                <div class="row">
                                    <div class="col-md-12">
                                ...
                                if(dt[i].from_id !== {{ Auth::user()->id }}){
                                    ...
                                    content += '<li class="left clearfix">'+
                                            '<span class="chat-img pull-left">'+
                                                '<img src="https://placehold.it/50/555658/fff&text='+(fromName.charAt(0)).toUpperCase()+'" alt="User Avatar" class="img-circle" />'+
                                            '</span>'+
                                    ...
                                    content += '<li class="right clearfix">'+
                                            '<span class="chat-img pull-right">'+
                                                '<img src="https://placehold.it/50/AE4435/fff&text='+(fromName.charAt(0)).toUpperCase()+'" alt="User Avatar" class="img-circle" />'+
                                            '</span>'+
                                            '<div class="chat-body clearfix">'+
                                                '<div class="header">'+
                                                ...
                                                '<p class="text-center p-50px-t">'+
                                                    '<div class="p-25px text-center">'+
                                                        '<div class="avatar-80 border-radius-50 d-inline-block">'+
                                                            '<img src="@if($role!=="admin") @if($role==="apl") {{  Auth::user()->hasApl()?\App\Models\User::find(Auth::user()->apl_id)->imageUrl():'' }} @else @if(Auth::user()->hasAfa()) {{  Auth::user()->hasAfa()?\App\Models\User::find(Auth::user()->afa_id)->imageUrl():'' }} @else {{  \App\Models\User::where("name",Request::get("afa"))->first()->imageUrl() }} @endif @endif @else {{ \App\Models\User::where("role",1)->first()->imageUrl() }} @endif" title="" alt="">'+
                                                        '</div>'+
                                                        '<h6 class="font-w-500 m-15px-t m-0px"><span class="font-w-700"> @if($role=="admin") {{ App\Models\User::where("id",1)->first()->name }} @elseif($role=="afa") @if(Auth::user()->hasAfa()) {{ App\Models\User::where("id",Auth::user()->afa_id)->first()->name }} @else {{ App\Models\User::where("name",Request::get("afa"))->first()->name }} @endif @else {{ App\Models\User::where("id",Auth::user()->apl_id)->first()->name }} @endif </span></h6>'+

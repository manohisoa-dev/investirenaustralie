    ...
    <div class="hover-top card box-shadow-only-hover overflow-hidden border-radius-0">
        ...
        <div class="p-20px">
            <label class="font-small">
				@lang('app.txt.postepar') : 
				<a href="javascript:void(0)">{{$item->author ? $item->author->name : ''}}</a> 
				– {{$item->created_at ? $item->created_at->diffForHumans() : ''}}
			</label>
            <h5 class="m-10px-b font-w-600">
				<a title="{{$item->$title}}" class="dark-color" href="{{route('blog.index',$item->$slug)}}" target="_blank">{{str_limit($item->$title, 50, '...') }}</a>
			</h5>

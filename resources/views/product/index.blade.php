                ...
                <div class="col-lg-8">
                  ...
                  <section class="property-meta-wrapper common">
                    <div class="row m-15px-t">
                        ...
                        <div class="col-sm-6">
                          <a href="{{route('member.go.there', $item )}}" id="btn_go_there" value="{{ Session::has('engagement')?1:0 }}" class="m-btn m-btn-theme flex-shrink-0 col-md-12" title="@lang('app.txt.go_to_location')" @if(Auth::check()) {{ Auth::user()->isMove()?(Auth::user()->memberHasSendMr(1,Auth::user()->id,Auth::user()->afa->id)?'disabled':''):(Auth::user()->isCheckedDossierTransaction($item->id)?'disabled':'') }} @endif><i class="fa fa-map-marker"></i> @lang('app.btn.go_to_location')</a>
                        </div>
                    </div>
                  </section>

...
class ChartController extends Controller
{
    ...
    public function sellers(Request $request)
    {
        ...
        $items = User::where('role', 2)
            ->join('products', 'products.seller_id', '=', 'users.id')
            ->join('type_users', 'type_users.id', '=', 'users.type_users_id')
            ->select(DB::raw('count(*) as number, type_user_name'))
            ->groupBy('type_users_id')
            ->get();
        for($i=0; $i<count($items); $i++){
            $item = $items[$i];
            if($item->type_users_id == 1){
                $item->color="#FF9E01";
            }else if($item->type_users_id == 2){
                $item->color="#04D215";
            }
        }

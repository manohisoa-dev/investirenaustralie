...
class ProfileController extends Controller {
    ...
    public function index() {
        ...
        return $view->with('title', __('app.profile'))->with('item', Auth::user())->with('breadcrumbs',
            __('app.profile'));
    }
    ...
    public function profile() {
        ...
        $breadcrumbs = [['active' => false, 'route' => route('profile'), 'label' => __('app.profile'), ], ['active' => true,
    ...
    public function editProfile(Request $request) {
        ...
        $default = ['name' => 'required|max:100', 'email' => 'required|email|max:100',
        ...
        switch ($role) {
            ...
            case 2:
                ...
                if ($user->TypeUser->type_user_name == 'Builder' || $user->TypeUser->type_user_name ==
                    'Developer') {
                    ...
                    if ($user->TypeUser->type_user_name == 'Organization') {
            ...
            case 1: // Administrateur
                $rules = ['email' => 'required|unique:users,email|max:100', 'language' =>
                    'required|max:100', 'first_name' => 'required|max:100', 'last_name' =>
                    'required|max:100', ];
                break;
            default:
                abort(404);
    ...
    public function updateMemberPartComplete(Request $request, $user, $role) {
        ...
        $rules = ['name' => 'required|max:100', 'email' => 'required|max:100',
            'language' => 'required|max:100', 'image' =>
            'image|mimes:jpeg,png,jpg,gif,svg|max:2048', 'country' => 'required|max:100',
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                // Créer user membre
                $users = ['name' => $datas['name'], 'email' => $datas['email'], 'language' => $datas['language'],
                    'image_id' => $datas['image_id'], 'is_complete'=>0];
                User::whereId($user->id)->update($users);

                //Créer localisation
                $locations = ['country' => $datas['country'], 'route' => $datas['route'],
                    'route_number' => $datas['route_number'], 'area_level_2' => $datas['area_level_2'],
                    'postalCode' => $datas['postalCode'], 'adrphy_country' => $datas['adrphy_country'],
                ...
                $userinfos = ['civility' => $datas['civility'], 'first_name' => $datas['first_name'],
                    'last_name' => $datas['last_name'], 'nationality' => $datas['nationality'],
                    'orga_phone' => $datas['orga_phone'], 'orga_mobile_phone' => $datas['orga_mobile_phone'],
                    'orga_email' => $datas['orga_email'], 'orga_fb' => $datas['orga_fb'], ];
                ...
                if (Session()->get('complete_registration')) {
                    ...
                    $member_name = $user->isPerson() ? $user->userinfos->first_name . ' ' . $user->userinfos->last_name :
                    $user->userinfos->orga_name;
                    ...
                    if($user->isMove()){
                        ...
                        DossierTransaction::whereId($request->id_doss)->update(['status' => 7,
                            'date_complete_profil' => Carbon::now()]);
                    }else{
                        // No deplacement
                        // Update dossier transaction
                        DossierTransaction::whereId($request->id_doss)->update(['status' => 1,
                            'date_complete_profil' => Carbon::now()]);

                        // get url product to buy
                        $prodUrl = url('product/'.$item->slug);
                        $dt=Carbon::now();
    ...
    public function updateMemberOrganization(Request $request, $user, $role) {
        ...
        $rules = ['name' => 'required|max:100', 'email' => 'required|max:100',
            'language' => 'required|max:100', 'image' =>
            'image|mimes:jpeg,png,jpg,gif,svg|max:2048', 'orga_phone' => 'required|max:100',
            'orga_fax' => 'nullable|max:100', 'orga_mobile_phone' => 'required|max:100',
            'orga_name' => 'required|max:100', 'orga_registration_number' =>
            'required|max:100', 'orga_rep_official_registration' => 'nullable|max:100',
            // 'orga_type'         => 'required',
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $userinfos = ['orga_phone' => $datas['orga_phone'], 'orga_fax' => $datas['orga_fax'],
                    'orga_mobile_phone' => $datas['orga_mobile_phone'], 'orga_name' => $datas['orga_name'],
                    ...
                    'orga_presentation' => $datas['orga_presentation'], 'contact_name' => $datas['contact_name'],
                    'contact_phone' => $datas['contact_phone'], 'contact_email' => $datas['contact_email'],
                    'allow_sharing' => $datas['allow_sharing'], 'newsletter' => $datas['newsletter'], ];
                Userinfo::where('user_id', $user->id)->update($userinfos);
    ...
    public function updateAfa(Request $request, $user, $role) {
        ...
        $rules = ['name' => 'required|max:100', 'email' => 'required|max:100',
            'language' => 'required|max:100', 'image' =>
            'image|mimes:jpeg,png,jpg,gif,svg|max:2048', 'orga_name' => 'required|max:100',
            'orga_trading_name' => 'required|max:100', 'orga_abn' =>
            'required|digits_between:11,11|numeric', 'orga_acn' =>
            'nullable|digits_between:9,9|numeric', 'orga_license_number' =>
            'required|max:100', // 'orga_email'        => 'required|email|max:100',
            'orga_phone' => 'required|digits_between:6,15|numeric', 'orga_fax' =>
            'nullable|max:100', 'orga_mobile_phone' =>
            'required|digits_between:6,15|numeric', 'orga_website' => 'required|url|max:100',
            'orga_presentation' => 'max:2000', 'orga_operation_state' => 'required',
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                //Créer localisation
                $locations = ['country' => $datas['afa_country'], 'area_level_1' => $datas['afa_area_level_1'],
                    'area_level_2' => $datas['afa_area_level_2'], 'postalCode' => $datas['afa_postalCode'],
                    'route' => $datas['route'], 'route_number' => $datas['afa_route_number'],
                    'locality' => $datas['afa_locality'], 'adrpost_postal_box' => isset($datas['adrpost_postal_box']) ?
                    $datas['adrpost_postal_box'] : '', 'adrpost_locality' => isset($datas['adrpost_locality']) ?
                ...
                $userinfos = ['orga_name' => $datas['orga_name'], 'orga_trading_name' => $datas['orga_trading_name'],
                    'orga_abn' => $datas['orga_abn'], 'orga_acn' => $datas['orga_acn'],
                    'orga_license_number' => $datas['orga_license_number'], 'orga_phone' => $datas['orga_phone'],
                    'orga_fax' => $datas['orga_fax'], 'orga_mobile_phone' => $datas['orga_mobile_phone'],
                    'orga_website' => $datas['orga_website'], 'orga_presentation' => $datas['orga_presentation'],
                    'orga_operation_state' => serialize($datas['orga_operation_state']),
                    'orga_operation_range' => $datas['orga_operation_range'], 'contact_name' => $datas['contact_name'],
                    'contact_email' => $datas['contact_email'], 'contact_phone' => $datas['contact_phone'], ];
                Userinfo::where('user_id', $user->id)->update($userinfos);
    ...
    public function updateApl(Request $request, $user, $role) {
        ...
        $rules = ['name' => 'required|max:100', 'email' => 'required|max:100',
            ...
            // 'orga_type'         => 'required',
            'orga_license_number' => 'required|max:100', 'orga_operation_range' =>
            ...
            'bank_agency' => 'required|max:100', 'bank_postal_box' => 'required|max:100',
            'bank_locality' => 'required|max:100', 'bank_postalCode' => 'required|max:100',
            'bank_country' => 'required|max:100', 'bank_iban' =>
            'required|alpha_num|min:27|max:27', 'bank_bic' =>
            'required|alpha_num|min:8|max:11', ];
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $userinfos = ['contact_name' => $datas['contact_name'], 'contact_phone' => $datas['contact_phone'],
                    'contact_email' => $datas['contact_email'], 'orga_name' => $datas['orga_name'],
                    'orga_registration_number' => $datas['orga_registration_number'],
                    'orga_rep_official_registration' => $datas['orga_rep_official_registration'],
                    'orga_license_number' => $datas['orga_license_number'], 'orga_operation_range' =>
                    $datas['orga_operation_range'], 'orga_presentation' => $datas['orga_presentation'],
                    'bank_name' => $datas['bank_name'], 'bank_agency' => $datas['bank_agency'],
                    'bank_iban' => $datas['bank_iban'], 'bank_bic' => $datas['bank_bic'], ];
                Userinfo::where('user_id', $user->id)->update($userinfos);
    ...
    public function updateSellerRep(Request $request, $user, $role) {
        ...
        $rules = ['name' => 'required|max:100', 'email' => 'required|max:100',
            ...
            'orga_trading_name' => 'required|max:100', 'orga_abn' =>
            'required|digits_between:11,11|numeric', 'orga_acn' =>
            'nullable|digits_between:9,9|numeric',
            // 'orga_email'        => 'required|email|max:100',
            'orga_phone' => 'required|digits_between:8,9|numeric', 'orga_fax' =>
            'nullable|max:100', 'orga_mobile_phone' => 'required|digits_between:9,9|numeric',
            'orga_website' => 'required|url|max:100', 'orga_presentation' => 'max:2000',
        ...
        if (isset($request->orga_parent_name)) {
            $rules += ['orga_parent_name' => 'max:100', ];
        }
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $userinfos = ['orga_parent_name' => isset($datas['orga_parent_name']) ? $datas['orga_parent_name'] :
                    '', 'orga_name' => $datas['orga_name'], 'orga_trading_name' => $datas['orga_trading_name'],
                    'orga_abn' => $datas['orga_abn'], 'orga_acn' => $datas['orga_acn'], 'orga_phone' =>
                    $datas['orga_phone'], 'orga_fax' => $datas['orga_fax'], 'orga_mobile_phone' => $datas['orga_mobile_phone'],
                    'orga_website' => $datas['orga_website'], 'orga_presentation' => $datas['orga_presentation'],
                    'contact_name' => $datas['contact_name'], 'contact_phone' => $datas['contact_phone'],
    ...
    public function updateSellerSnp(Request $request, $user, $role) {
        ...
        $rules = ['name' => 'required|max:100', 'email' => 'required|max:100',
            'language' => 'required|max:100', 'image' =>
            'image|mimes:jpeg,png,jpg,gif,svg|max:2048', // Seller #1
            'last_name' => 'required|max:100', 'first_name' => 'required|max:100',
            'date_of_birth' => 'required|max:100', 'place_of_birth' => 'required|max:100',
            'nationality' => 'required|max:100', 'street_adr' => 'required|max:100',
            'suburb' => 'required|max:100', 'city' => 'required|max:100', 'post_code' =>
            'required|max:100', 'state' => 'nullable|max:100', 'country' =>
            'required|max:100', // 'phone' => 'required|digits_between:6,9|numeric',
            'mobile' => 'required|digits_between:6,9|numeric', 'email_adr' =>
            'required|email|max:100', // Seller #2
            'last_name_2' => 'nullable|max:100', 'first_name_2' => 'nullable|max:100',
            'date_of_birth_2' => 'nullable|max:100', 'place_of_birth_2' =>
            'nullable|max:100', 'nationality_2' => 'nullable|max:100', 'street_adr_2' =>
            'nullable|max:100', 'suburb_2' => 'nullable|max:100', 'city_2' =>
            'nullable|max:100', 'post_code_2' => 'nullable|max:100', 'state_2' =>
            'nullable|max:100', 'country_2' => 'nullable|max:100',
            // 'phone_2' => 'nullable|digits_between:6,9|numeric',
            'mobile_2' => 'nullable|digits_between:6,9|numeric', 'email_adr_2' =>
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $dt = $dt->toDateString();
                $sellerIndividuals_1 = ['last_name' => $datas['last_name'], 'first_name' => $datas['first_name'],
                    'date_of_birth' => $dt, 'place_of_birth' => $datas['place_of_birth'],
                    'nationality' => $datas['nationality'], 'street_adr' => $datas['street_adr'],
                    'suburb' => $datas['suburb'], 'city' => $datas['city'], 'post_code' => $datas['post_code'],
                    'state' => $datas['state'], 'country' => $datas['country'], 'mobile' => $datas['mobile'],
                ...
                $sellerIndividuals_2 = ['last_name' => isset($datas['last_name_2']) ? $datas['last_name_2'] :
                    '', 'first_name' => isset($datas['first_name_2']) ? $datas['first_name_2'] : '',
                    'date_of_birth' => $dt, 'place_of_birth' => isset($datas['place_of_birth_2']) ?
                    $datas['place_of_birth_2'] : '', 'nationality' => isset($datas['nationality_2']) ?
                    $datas['nationality_2'] : '', 'street_adr' => isset($datas['street_adr_2']) ? $datas['street_adr_2'] :
                    '', 'suburb' => isset($datas['suburb_2']) ? $datas['suburb_2'] : '', 'city' =>
                    isset($datas['city_2']) ? $datas['city_2'] : '', 'post_code' => isset($datas['post_code_2']) ?
                    $datas['post_code_2'] : '', 'state' => isset($datas['state_2']) ? $datas['state_2'] :
                    '', 'country' => isset($datas['country_2']) ? $datas['country_2'] : '', 'mobile' =>
                    isset($datas['mobile_2']) ? $datas['mobile_2'] : '', 'email_adr' => isset($datas['email_adr_2']) ?
                    $datas['email_adr_2'] : ''];
    ...
    public function updateSellerSbaIndividual(Request $request, $user, $role) {
        ...
        $rules = ['name' => 'required|max:100', 'email' => 'required|max:100',
            'language' => 'required|max:100', 'image' =>
            'image|mimes:jpeg,png,jpg,gif,svg|max:2048', 'contact_name' =>
            'required|max:100', 'contact_email' => 'required|email|max:100', 'contact_phone' =>
            'required|digits_between:6,9|numeric', // Seller #1
            'last_name' => 'required|max:100', 'first_name' => 'required|max:100',
            'street_adr' => 'required|max:100', 'suburb' => 'required|max:100', 'city' =>
            'required|max:100', 'post_code' => 'required|max:100', 'state' =>
            'nullable|max:100', 'country' => 'required|max:100', 'mobile' =>
            'required|digits_between:6,9|numeric', 'email_adr' => 'required|email|max:100',
            // Seller #2
            'last_name_2' => 'nullable|max:100', 'first_name_2' => 'nullable|max:100',
            'street_adr_2' => 'nullable|max:100', 'suburb_2' => 'nullable|max:100', 'city_2' =>
            'nullable|max:100', 'post_code_2' => 'nullable|max:100', 'state_2' =>
            'nullable|max:100', 'country_2' => 'nullable|max:100', 'mobile_2' =>
            'nullable|digits_between:6,9|numeric', 'email_adr_2' => 'nullable|email|max:100', ];
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $userinfos = ['contact_name' => $datas['contact_name'], 'contact_email' => $datas['contact_email'],
                    'contact_phone' => $datas['contact_phone'], ];
                Userinfo::where('user_id', $user->id)->update($userinfos);

                //Update localisation

                $locations = ['country' => $datas['country'], 'area_level_1' => $datas['state'],
                    'area_level_2' => $datas['suburb'], 'postalCode' => $datas['post_code'], 'route' =>
                    $datas['street_adr'], 'locality' => $datas['city'], 'latitude' => $datas['sba_lat'],
                    'longitude' => $datas['sba_long'], ];
                Localisation::whereId($user->location_id)->update($locations);
                ...
                $idSeller = $datas['id_seller'];
                $sellerIndividuals_1 = ['last_name' => $datas['last_name'], 'first_name' => $datas['first_name'],
                    'street_adr' => $datas['street_adr'], 'suburb' => $datas['suburb'], 'city' => $datas['city'],
                    'post_code' => $datas['post_code'], 'state' => $datas['state'], 'country' => $datas['country'],
                    'mobile' => $datas['mobile'], 'email_adr' => $datas['email_adr']];
                SellerIndividual::whereId($idSeller)->update($sellerIndividuals_1);

                // Seller #2
                $idSeller2 = $datas['id_seller_2'];
                $sellerIndividuals_2 = ['last_name' => $datas['last_name_2'], 'first_name' => $datas['first_name_2'],
                    'street_adr' => $datas['street_adr_2'], 'suburb' => $datas['suburb_2'], 'city' =>
                    $datas['city_2'], 'post_code' => $datas['post_code_2'], 'state' => $datas['state_2'],
                    'country' => $datas['country_2'], 'mobile' => $datas['mobile_2'], 'email_adr' =>
                    $datas['email_adr_2']];
                SellerIndividual::whereId($idSeller2)->update($sellerIndividuals_2);
    ...
    public function updateSellerSbaBusiness(Request $request, $user, $role) {
        ...
        $rules = ['name' => 'required|max:100', 'email' => 'required|max:100',
            ...
            'image|mimes:jpeg,png,jpg,gif,svg|max:2048', 'contact_name' =>
            'required|max:100', 'contact_email' => 'required|email|max:100', 'contact_phone' =>
            'required|digits_between:6,9|numeric', 'business_name' => 'required|max:100',
            'business_parent' => 'nullable|max:191', 'street_adr' => 'required|max:100',
            'suburb' => 'required|max:100', 'city' => 'required|max:100', 'post_code' =>
            'required|max:100', 'state' => 'required|max:100', 'country' =>
            'required|max:100', 'phone' => 'required|digits_between:6,9|numeric', 'mobile' =>
            'required|digits_between:6,9|numeric', 'email_adr' => 'required|email|max:100',
            'contact_name' => 'required|max:100', 'contact_email' =>
            'required|email|max:100', 'contact_phone' =>
            'required|digits_between:6,9|numeric', ];
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $sellerBusiness = ['business_name' => $datas['business_name'], 'business_parent' =>
                    $datas['business_parent'], 'street_adr' => $datas['street_adr'], 'suburb' => $datas['suburb'],
                    'city' => $datas['city'], 'post_code' => $datas['post_code'], 'state' => $datas['state'],
                    'country' => $datas['country'], 'mobile' => $datas['mobile'], 'phone' => $datas['phone'],
                    'email_adr' => $datas['email_adr']];
    ...
    public function updatePassword(Request $request) {
        ...
        $validator = Validator::make($request->all(), ['old_password' =>
            'required|min:8', 'password' => 'required|min:8', 'password_confirmation' =>

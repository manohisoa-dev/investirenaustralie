 ...
 * 
 */
if (!function_exists('nouvelle_afa_disponible')) {
    function nouvelle_afa_disponible() {
        try {
...
if (!function_exists('update_afa_disponible')) {
    function update_afa_disponible($prod_id) {
        try {
            $produits = App\Models\Product::whereId($prod_id)->get();
 ...
 * 
 */
if (!function_exists('distance_point')) {
    function distance_point($lat1, $lon1, $lat2, $lon2, $unit) {
        $theta = $lon1 - $lon2;
